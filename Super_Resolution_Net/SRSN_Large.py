...
class SRSN(nn.Module):
    def __init__(self, input_dim=3, dim=128, scale_factor=4):
        ...
        self.conv256to128 = torch.nn.Conv2d(256, 128, 1, 1, 0)
        self.conv512to128 = torch.nn.Conv2d(512, 128, 1, 1, 0)
...
#         print(out.shape)
        out1=torch.cat((out, LR_feat), 1)
#         print(out1.shape)
        out1=self.conv256to128(out1)
#         print(out1.shape)
#         out1 = out + LR_feat
        out1 = self.resnet2(out1)
#         out1 = out + out1
        out1=torch.cat((out, out1), 1)
        out1=self.conv256to128(out1)
        out2 = self.resnet3(out1)
#         out2 = out + out2
        out2=torch.cat((out, out2), 1)
        out2=self.conv256to128(out2)

        out3 = self.resnet4(out2)
#         out3 = out+out1+out2 + out3
        out3=torch.cat((out, out1,out2,out3), 1)
        out3=self.conv512to128(out3)
#         print(out3.shape)
